...
*/
	...
	tip = d3.select("#toolwrap")
		.style("display", "block")
		.style("bottom", +(r)+height()-d.y-3+"px")
		;
...
function nodeout(d, i) {
	console.log("node")
	if ($('#toolwrap:hover').length == 0) d3.select("#toolwrap").style("display", "none");
	unfix(d);
}
...
function countyover(d, i){
	...
	tip = d3.select("#countytip")
		.style("display", "block")
		.style("bottom", d3.event.x+"")
...
function insert_map(dataName, elementID, sourceName) {
	...
	if(!((dataName + elementID) in maps)) {
		...
		var path = d3.geoPath().projection(d3.geoAlbersUsa().scale(450).translate([width/2-35,height/2]));
			...
			d3.json("us-states.json", function(errort, states){
				...
				$.ajax({
					...
					.attr('opacity', function(d,i){
						if(d.id in data) {
							return (data[d.id][indexer]-min)/(max-min);	
						}
					})
					//.attr('fill', color(sourceName));
					.attr('fill', "rgb(0,0,0)")
					.on("mouseover", countyover)
					.on("mouseleave", countyleave)
					;
